...
async function seedSampleData() {
  try {
    ...
    // Cek apakah demo users sudah ada (berdasarkan email)
    const adminExists = Number(
      (
        await q(
          db.prepare(
            "SELECT COUNT(*) as c FROM users WHERE email = 'demo@mail.com'",
          ),
          "get",
        )
      ).c,
    ...
    if (adminExists === 0) {
      // Hapus seluruh user lama agar benar-benar bersih hanya ada 1 user demo
      try {
        await q(db.prepare("DELETE FROM users"), "run");
      } catch (e) {
        console.warn("[DB] Hapus user lama gagal:", e.message);
      }
      ...
      await q(insUser, "run", "Demo Admin", "demo@mail.com", "admin", null, "", hashedPassword);

      console.log(
        `[DB] Seeded 1 demo user account (demo@mail.com / demo123)`
      );
    }
  } catch (err) {
    console.warn("[DB] seedSampleData gagal:", err.message);
  }
}
